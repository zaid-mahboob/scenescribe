    ...
    try:
        with sr.Microphone(device_index=0) as source:
            ...
            while True:
                ...
                if ref == 1:
                    try:
                        # Continuously listen and accumulate audio while the
                        # button is pressed
        ...
        if accumulated_audio:
            ...
            combined_audio = b"".join(
                [segment.get_wav_data() for segment in accumulated_audio]
            )

            # Convert the combined audio into an AudioData object with the same
            # sample rate and sample width as the original
            full_audio = sr.AudioData(
                combined_audio,
                accumulated_audio[0].sample_rate,
                accumulated_audio[0].sample_width,
            )

            try:
                # Pass the full accumulated audio to Google Speech API for
                # recognition
