...
def record_with_firebase_control(
    output_path="recorded_audio.wav", sample_rate=16000, frame_duration=30
):
...
def record_video_from_camera(
    duration=5,
    fps=2,
    output_filename="video_smolvlm.avi",
    resolution=(640, 480),
    video_dir="/home/scenescribe/Desktop/scenescribe/avis",
):
    """
...
def activity_detection(user_input):
    try:
        ...
        match = re.search(
            r'Assistant: (.*?)"\s*,\s*"processing_time_seconds', full_response
        )

        if match:
            # Decode the assistant's response correctly
            assistant_answer = match.group(1).encode(
            ...
            raise ValueError(
                "Could not find the assistant's answer in the response text."
            )
...
def video_agent_2(user_input, agent_1_output):
    ...
    if language is not None:
        prompt = prompt + " " + \
            f"Please give final ouput in {language} language"
...
def navigation_agent_2(user_input, agent_1_output):
    ...
    if language is not None:
        prompt = prompt + " " + \
            f"Please give final ouput in {language} langauge"
    messages = [{"role": "user", "content": prompt}]
    print("Content Prepared")
    # Call OpenAI API with image and text input, including conversation history
    completion = openai.chat.completions.create(
