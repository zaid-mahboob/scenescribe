...
class Utils:
    def __init__(
        ...
        self.whisper_model = (
            whisper_model if whisper_model else whisper.load_model("tiny")
        )
    ...
    def analyze_video_with_prompt(
        self, video_path=None, prompt_text=video_prompt, endpoint_url=endpoint_url
    ):
        """
    ...
    def openai_convert_and_play_speech(self, text):
        ...
        response = self.openai.audio.speech.create(
            ...
            voice="alloy",
            # You can try "echo", "nova", "fable", "shimmer" etc.
            input=text,
        )
        ...
        # Increase volume by 1.5x (can go higher with a risk of voice change,
        # but avoid clipping)
        volume_boost = 1.5
    ...
    def record_with_softap_control(
        self, output_path="recorded_audio.wav", sample_rate=16000, frame_duration=30
    ):
